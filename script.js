document.addEventListener("DOMContentLoaded", () => {
  ...
  // Update menu on resize and load
...
console.log('Testimony section script loaded at', new Date().toLocaleString());
...
// Initialize with original testimonial as a comment
const originalTestimonial = {
  name: 'Akinsuroju Similoluwa, IT Project Manager',
  text: 'Thank you for your amazing support during the tech festival! You were my go-to problem solver, and your calm under pressure made all the difference. The event was a success because of you, and I’m forever grateful!',
  timestamp: '2025-07-24T00:00:00Z' // Fixed timestamp for consistency
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!isLocalStorageAvailable()) {
    console.warn('localStorage unavailable, comments will not persist');
    if (storageError) storageError.style.display = 'block';
    renderComment(originalTestimonial, commentsList); // Render original testimonial
    return;
  }
  // Initialize localStorage with original testimonial if empty
  let comments = JSON.parse(localStorage.getItem('comments') || '[]');
  if (!comments.some(c => c.name === originalTestimonial.name && c.text === originalTestimonial.text)) {
    comments = [originalTestimonial, ...comments];
    localStorage.setItem('comments', JSON.stringify(comments));
  }
  console.log('Loaded comments from localStorage:', comments);
...
if (commentForm) {
  commentForm.addEventListener('submit', (e) => {
    ...
    if (name && text) {
      if (!isLocalStorageAvailable()) {
        console.warn('Cannot save comment: localStorage unavailable');
        if (storageError) storageError.style.display = 'block';
        renderComment({ name, text, timestamp: new Date().toISOString() }, document.getElementById('comments-list')); // Render temporarily
        return;
      }
      const comment = { name, text, timestamp: new Date().toISOString() };
      console.log('Saving comment:', comment);
      saveComment(comment);
      renderComment(comment, document.getElementById('comments-list'));
      commentForm.reset();
    } else {
      console.warn('Empty name or comment text');
    }
  });
} else {
...
function renderComment(comment, container) {
  ...
  commentDiv.innerHTML = `
    <p class="comment-name">${sanitizeHTML(comment.name)}</p>
    <p class="comment-text">${sanitizeHTML(comment.text)}</p>
  `;
